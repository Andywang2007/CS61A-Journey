...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        """Return a random Bee from the nearest Place 
        (excluding the Hive) that contains Bees and is reachable from
        ...
        while current is not None:
            if not current.is_hive and count <= self.upper_bound and count >=self.lower_bound and current.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, damage_taken: float):
        ...
        "*** YOUR CODE HERE ***"
        place = self.place
        damagetodeal = damage_taken
        if self.health <= damage_taken:
            damagetodeal += self.damage
        for bee in place.bees[:]:
            bee.reduce_health(damagetodeal)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate: GameState):
        if self.cooldown != 0:
            ...
            if self.place.bees:
                unluckbee = random_bee(self.place.bees)
                unluckbee.reduce_health(unluckbee.health)
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self, health=1):
        super().__init__(health)
        if not hasattr(QueenAnt, 'true_queen_created'):
            QueenAnt.true_queen_created = False

        if not QueenAnt.true_queen_created:
    ...
    def double_ant(self, ant):
        if ant is not None and hasattr(ant, 'damage') and not getattr(ant, 'is_doubled', False):
            ant.damage *= 2
            ant.is_doubled = True
